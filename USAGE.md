...
|---|---|---|
| GET | `/` | 控制台页面 |
| GET | `/healthz` | `{ok, nodes, bootId, startedAt, pid}` —— `bootId` 用来确认你连的是**新**进程 |
...
**当前测试规模**：`npm test` **217 个用例 / 18 个文件**（209 通过 + 8 个需要真子进程的用例在受限沙箱内
...
**开发过程中被抓出并修复的真实缺陷：62 个**，及各自的触发条件，全部记在
